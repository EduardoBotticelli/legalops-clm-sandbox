...
# --- 3. UI & CSS PREMIUM (ESTILO BCHECK) ---
...
with tab_inbound:
    ...
    with col_upload:
        st.markdown('<div class="panel-card"><div class="panel-title">Auditoria Semântica</div>', unsafe_allow_html=True)
        ...
        if st.session_state.get('run_ia', False) and up_pdf:
            with st.spinner("Analisando estrutura do contrato..."):
                try:
                    ...
                    # Lógica de seleção de modelo original (segura)
                    models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
                    sel_model = next((m for m in models if 'flash' in m.lower()), models[0])
                    
                    res = genai.GenerativeModel(sel_model).generate_content(prompt)
                    ...
                    for a in dados.get("achados", []):
                        cor_h = "#EF4444" if "ALTA" in a.get("gravidade", "").upper() else "#3B82F6"
...
# --- MÓDULO 2: OUTBOUND (GERAÇÃO) ---
with tab_outbound:
    st.markdown('<div class="panel-card"><div class="panel-title">1. Template Master (.docx)</div>', unsafe_allow_html=True)
    up_docx = st.file_uploader("", type="docx")
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    if up_docx:
        ...
        if not variaveis:
            ...
            with col_left:
                with st.form("f_gen"):
                    st.markdown('<div class="panel-title" style="border:none;">2. Preenchimento</div>', unsafe_allow_html=True)
                    contexto = {}
                    for var in sorted(variaveis):
                        var_up = var.upper()
                        if "CPF" in var_up or "CNPJ" in var_up:
                            # SELETOR ORIGINAL QUE VOCÊ GOSTAVA
                            ...
                            contexto[var] = st.number_input(f"💰 {var.title()}", format="%.2f")
                        else:
            ...
            if 'out_bytes' in st.session_state:
                with col_right:
                    ...
                    st.download_button("📥 Baixar Arquivo", st.session_state['out_bytes'], "Contrato_Final.docx")
                    st.markdown('</div>', unsafe_allow_html=True)
